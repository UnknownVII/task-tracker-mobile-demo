...
late List<UserData> user_data;
late RegisterRequestModel registerRequestModel = new RegisterRequestModel(name: '', email: '', password: '');
late LoginRequestModel loginRequestModel = new LoginRequestModel(email: '', password: '');
late UseRequestModel useRequestModel = new UseRequestModel(userID: '', token: '');
  ...
  await loginService.login(loginRequestModel).then(
    (_userData) async {
      ...
      if (_userData.authToken.isNotEmpty) {
        sharedPreferences.setString('data', loginRequestModel.toJson().toString());
        sharedPreferences.setString('authKey', _userData.authToken.toString());
        sharedPreferences.setString('currentUser', _userData.message.toString());
        sharedPreferences.setString('currentEmail', loginRequestModel.email.toString());
...
Future getUserSpecificData() async {
  List<dynamic> value = [];
  ...
  useRequestModel.userID = userID!;
  useRequestModel.token = authKey!;

  await userDataService.getData(useRequestModel).then(
        (_userData) {
      if (_userData.user_data.isNotEmpty) {
        value.addAll(_userData.user_data);
        user_data = _userData.user_data;
      }
      if (_userData.error.isNotEmpty) {
        value.add(_userData.error.replaceAll('\n', ''));
      }
